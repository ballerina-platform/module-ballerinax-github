...
remote function GitHubConnector.createIssue(string repositoryOwner, string repositoryName, string issueTitle,
    ...
    if (repositoryName == EMPTY_STRING || repositoryOwner == EMPTY_STRING || issueTitle == EMPTY_STRING) {
        error connectorError = error(GITHUB_ERROR_CODE, { message: "Repository name, owner and issue title should be specified" });
...
remote function GitHubConnector.getCardListNextPage(CardList cardList) returns CardList|error {
...
remote function GitHubConnector.getColumnListNextPage(ColumnList columnList) returns ColumnList|error {
...
remote function GitHubConnector.getIssueList(Repository|(string, string) repository, string state, int recordCount)
                                     returns IssueList|error {
...
remote function GitHubConnector.getIssueListNextPage(IssueList issueList) returns IssueList|error {
...
remote function GitHubConnector.getOrganization(string name) returns Organization|error {
    ...
    if (jsonValidatedResponse is json) {
        ...
        var value = Organization.create(githubRepositoryJson);
...
remote function GitHubConnector.getOrganizationProject(Organization|string organization, int projectNumber)
                                     returns Project|error {
    ...
    if (organizationName == EMPTY_STRING || projectNumber <= INDEX_ZERO) {
        error err = error(GITHUB_ERROR_CODE, { message: "Organization should have a name and project number should be positive integer" });
        return err;
    }
    ...
    if (jsonValidatedResponse is json) {
        ...
        var result = Project.create(githubProjectJson);
        if (result is Project) {
            return result;
        } else {
            error err = error(GITHUB_ERROR_CODE, { message: "Error occurred while accessing the Project payload of the response." });
            return err;
        }
    } else {
        error err = error(GITHUB_ERROR_CODE, { message: "Error occurred while accessing the Json payload of the response." });
...
remote function GitHubConnector.getOrganizationProjectList(Organization|string organization, string state,
    ...
    if (organizationName == EMPTY_STRING || state == EMPTY_STRING) {
        error err = error(GITHUB_ERROR_CODE, { message: "Organization should have a name and project state should be specified" });
        return err;
    }
...
remote function GitHubConnector.getOrganizationRepositoryList(Organization|string organization, int recordCount)
                                     returns RepositoryList|error {
...
remote function GitHubConnector.getProjectColumnList(Project project, int recordCount) returns ColumnList|error {
...
remote function GitHubConnector.getProjectListNextPage(ProjectList projectList) returns ProjectList|error {
    http:Client gitHubEndpoint = self.githubGraphQlClient;
...
remote function GitHubConnector.getPullRequestList(Repository|(string, string) repository, string state, int recordCount)
                                     returns PullRequestList|error {
...
remote function GitHubConnector.getPullRequestListNextPage(PullRequestList pullRequestList)
                                     returns PullRequestList|error {
...
remote function GitHubConnector.getRepository(string name) returns Repository|error {
...
remote function GitHubConnector.getRepositoryListNextPage(RepositoryList repositoryList)
                                     returns RepositoryList|error {
    http:Client gitHubEndpoint = self.githubGraphQlClient;
...
remote function GitHubConnector.getRepositoryProject(Repository|(string, string) repository, int projectNumber)
                                     returns Project|error {
    ...
    if (jsonValidatedResponse is json) {
        ...
        return Project.create(githubProjectJson);
    } else {
        error err = error(GITHUB_ERROR_CODE, { message: "Error occurred while accessing the Json payload of the response." });
        return err;
    }
...
remote function GitHubConnector.getRepositoryProjectList(Repository|(string, string) repository, string state,
                                                          int recordCount) returns ProjectList|error {
